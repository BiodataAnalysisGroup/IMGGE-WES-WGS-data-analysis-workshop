...
# input of trim_galore step:  
  ...
  raw_fastq_file_2: 
    type: File
# input for bwa mem
  bwa_ref_genome: # secondary files required
    type: File
...
# input for bcftools mpileup
  bcftools_ref_genome: # secondary files required
    type: File
    format: edam:format_1929 # FASTA
    secondaryFiles: 
      - .fai
...
# bwa mem mapping reads:
  bwa_mem_mapping_reads:
    ...
    in: 
      ...
      Index: bwa_ref_genome
    out: [reads_stdout]
# SAM_to_BAM_conversion:
  sam_to_bam_conversion:
    run: ../wrappers/samtools_view_sam2bam.cwl
    in:
      sam: bwa_mem_mapping_reads/reads_stdout # sam_file
    out: [bam]
# BAM_sorting:
  bam_file_sorting:
    run: ../wrappers/samtools_sort.cwl
    in:
...
# bcftools_mpileup_step:
  bcftools_mpileup_step:
    run: ../wrappers/bcftools_mpileup.cwl
    in:
      refGenome: bcftools_ref_genome
      outputFileType: output_file_type
      inputBam: bam_file_sorting/bam_sorted # bam_file_2nd
    out: [pileupOut]
# bcftools_call_step:
  bcftools_call_step:
    run: ../wrappers/bcftools_call.cwl
    in:
