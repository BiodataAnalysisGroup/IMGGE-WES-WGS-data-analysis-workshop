...
bcftools filter -s QUAL100 -e '%QUAL<100' -Oz mother.vcf | bcftools filter -s DP5 -e 'FORMAT/DP<5' -m+ -Oz -o mother_filtered.vcf.gz
tabix -p vcf mother_filtered.vcf.gz
bcftools filter -s QUAL100 -e '%QUAL<100' -Oz proband.vcf | bcftools filter -s DP5 -e 'FORMAT/DP<5' -m+ -Oz -o proband_filtered.vcf.gz
